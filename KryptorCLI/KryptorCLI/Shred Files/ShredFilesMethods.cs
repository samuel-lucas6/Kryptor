    ...
    public static class ShredFilesMethods
    {
        public static void FirstLast16KiB(string filePath)
        {
            ...
            catch (Exception ex) when (ExceptionFilters.FileAccessExceptions(ex))
            {
                ...
                DisplayMessage.Error(filePath, ex.GetType().Name, "'First/Last 16KiB' erasure failed.");
            }
        }

        public static void PseudorandomData(string filePath)
        {
            try
            {
                ...
                {
                    byte[] randomBytes = FileHandling.GetBufferSize(fileStream.Length);
            ...
            {
                Logging.LogException(ex.ToString(), Logging.Severity.High);
                DisplayMessage.Error(filePath, ex.GetType().Name, "'1 Pass' erasure failed.");
            }
        }

        public static void ZeroFill(string filePath, bool useOnes)
        {
            try
            {
                using (var fileStream = new FileStream(filePath, FileMode.Open, FileAccess.ReadWrite, FileShare.Read, Constants.FileBufferSize, FileOptions.SequentialScan))
                {
                    byte[] zeroes = FileHandling.GetBufferSize(fileStream.Length);
                    if (useOnes == true)
                    {
            ...
            {
                Logging.LogException(ex.ToString(), Logging.Severity.High);
                DisplayMessage.Error(filePath, ex.GetType().Name, "'Zero fill' erasure failed.");
            }
        }
        ...
        public static void InfosecStandard5Enhanced(string filePath)
        {
            // First pass with zeros
            ZeroFill(filePath, false);
            // Second pass with ones
            ZeroFill(filePath, true);
            // Third pass with random data
            PseudorandomData(filePath);
        }

        public static void PseudorandomData5Passes(string filePath)
        {
            for (int i = 0; i < 5; i++)
            {
                PseudorandomData(filePath);
        ...
        public static void EncryptionErasure(string filePath)
        {
            try
            {
                ...
                using (var plaintext = new FileStream(filePath, FileMode.Open, FileAccess.ReadWrite, FileShare.Read, Constants.FileBufferSize, FileOptions.SequentialScan))
                {
                    ...
                    StreamCiphers.Encrypt(plaintext, ciphertext, fileBytes, nonce, key);
            ...
            {
                Logging.LogException(ex.ToString(), Logging.Severity.High);
                DisplayMessage.Error(filePath, ex.GetType().Name, "'Encryption' erasure failed.");
            }
        }
    }
}
